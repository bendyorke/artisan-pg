...
exports.drop = function() {
  ...
  pg.connect(conStringGlobal, function(err, client, done) {
    ...
    client.query('DROP DATABASE ' + databaseName, function(err) {
      if(err) throw err
      fs.unlink(CONFIG.schemaFile, function(err) { if(err) throw err })
      client.end()
    })
  })
}
...
exports.migrate = function(version) {
  ...
  fs
    ...
    .forEach(function(file) {
      ...
      fs.writeFileSync(CONFIG.schemaFile, 'exports.version = ' + (endVersion - 1) )
    })
  
  fs.writeFileSync(CONFIG.schemaFile, 'exports.version = ' + (argVersion !== null ? argVersion : (endVersion - 1)) )
}
